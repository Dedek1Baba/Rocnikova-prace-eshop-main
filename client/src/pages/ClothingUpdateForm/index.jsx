...
export default function ClothingUpdateForm() {
  ...
  const updateForm = async () => {
    const data = await updateClothing(id, formData, password);
    ...
    <div className="flex items-center justify-center min-h-screen p-6">
      <div className="bg-white shadow-xl rounded-3xl p-6 w-full max-w-md flex flex-col items-center gap-4">
        ...
        <form onSubmit={handleUpdate} className="flex flex-col gap-3 w-full">
          ...
          <input type="password" required placeholder="Admin password" onChange={(e) => setPassword(e.target.value)} className="p-3 border rounded-lg w-full" />

          <Button type="submit" className="bg-indigo-600 hover:bg-indigo-500 w-full py-4 text-white text-lg rounded-lg">
            Update Clothing
          </Button>
        ...
        <div className="grid grid-cols-2 gap-3 w-full mt-4">
          <Link to="/admin">
            <Button className="bg-gray-300 hover:bg-gray-400 text-gray-900 font-semibold w-full">Go back</Button>
          </Link>
          <Link to="/">
            <Button className="bg-gray-300 hover:bg-gray-400 text-gray-900 font-semibold w-full">Main Page</Button>
          </Link>
        </div>
      </div>
    </div>
